                ...
                @if($friends->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        @foreach($friends as $friend)
                            <div class="group relative flex items-center p-5 bg-white border border-gray-100 rounded-2xl hover:border-blue-200 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                                <!-- Avatar -->
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-lg shadow-orange-900/5 border border-white/80 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100/50 flex items-center bg-white/50">
                ...
                @if($pendingRequests->count() > 0)
                    <span class="ml-auto bg-orange-500 text-white text-[10px] font-bold px-2 py-1 rounded-full animate-pulse">{{ $pendingRequests->count() }}</span>
                @endif
            </div>
                ...
                @if($pendingRequests->count() > 0)
                    <div class="space-y-4">
                        @foreach($pendingRequests as $request)
                            <div class="p-4 bg-orange-50/50 border border-orange-100 rounded-2xl hover:bg-orange-50 transition-colors">
                ...
                @if($sentRequests->count() > 0)
                    <div class="space-y-3">
                        @foreach($sentRequests as $request)
                            <div class="flex items-center p-3 border border-gray-100 bg-gray-50/50 rounded-2xl">
